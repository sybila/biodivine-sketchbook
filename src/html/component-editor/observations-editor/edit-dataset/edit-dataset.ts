...
export default class EditDataset extends LitElement {
  ...
  @state() origData: IObservationSet | undefined
  id = ''
    ...
    return html`
      <div class="uk-container uk-margin-medium-bottom">
        <form class="uk-form-horizontal uk-flex uk-flex-column uk-flex-between">
          <div class="fields">
            <!-- Editable fields for dataset properties -->
              ...
              return html`
                <div class="uk-margin-small">
                  ...
                  <div class="uk-form-controls">
                    ${key === 'annotation'
                      ? html`<textarea class="uk-textarea" .value="${this.getValue(this.data, key)}" 
                                       @input="${(e: InputEvent) => { this.setValue(this.data, key, (e.target as HTMLTextAreaElement).value) }}" 
                                       id="dataset-${key}" placeholder="${key}"></textarea>`
                      : html`<input class="uk-input" .value="${this.getValue(this.data, key)}" 
                                    @input="${(e: InputEvent) => { this.setValue(this.data, key, (e.target as HTMLInputElement).value) }}" 
                                    id="dataset-${key}" type="text" placeholder="${key}"/>`}
                  </div>
                </div>`
            })}

            <!-- Editable fields for variable names -->
            <h4 class="uk-text-bold uk-margin-top">Variables</h4>
            ${map(this.origData?.variables ?? [], (origVariable, index) => {
              const currentVariable = this.data?.variables?.[index] ?? origVariable
              return html`
                <div class="uk-margin-small">
                  <label class="uk-form-label" for="form-variable-${origVariable}">${origVariable}</label>
                  <div class="uk-form-controls">
                    <input class="uk-input" .value="${currentVariable}"
                          @input="${(e: InputEvent) => {
                            if (this.data?.variables !== undefined) {
                              this.setValue(this.data.variables, index.toString(), (e.target as HTMLInputElement).value)
                            }
                          }}" 
                          id="form-variable-${origVariable}" type="text" placeholder="Variable Name"/>
                  </div>
                </div>`
            })}
          </div>
